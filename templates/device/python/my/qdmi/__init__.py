...
MY_QDMI_DATA = resolved_include_dir.parents[1]
assert MY_QDMI_DATA.exists(), f"MY_QDMI_DATA does not exist: {MY_QDMI_DATA}"

MY_QDMI_LIBRARY_DIR = MY_QDMI_DATA / "lib"
if not MY_QDMI_LIBRARY_DIR.exists():
    MY_QDMI_LIBRARY_DIR = MY_QDMI_DATA / "lib64"
assert MY_QDMI_LIBRARY_DIR.exists(), f"MY_QDMI_LIBRARY_DIR does not exist: {MY_QDMI_LIBRARY_DIR}"

# the library is the sole file in the lib directory
library_files = list(MY_QDMI_LIBRARY_DIR.glob("*my-qdmi-device*"))
if not library_files:
    msg = f"No MY QDMI library found in: {MY_QDMI_LIBRARY_DIR}"
    raise FileNotFoundError(msg)
MY_QDMI_LIBRARY_PATH = library_files[0]

MY_QDMI_INCLUDE_DIR = MY_QDMI_DATA / "include"
assert MY_QDMI_INCLUDE_DIR.exists(), f"MY_QDMI_INCLUDE_DIR does not exist: {MY_QDMI_INCLUDE_DIR}"

MY_QDMI_CMAKE_DIR = MY_QDMI_DATA / "share" / "cmake"
