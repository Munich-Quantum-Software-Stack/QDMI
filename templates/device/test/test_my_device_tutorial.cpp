 ...
 *
 * Tutorial test suite – generated automatically from docs/tutorial.md.
 * Enabled in the template via -DTEMPLATE_TEST_SUITE=ON.
 */
...
protected:
  ...
  void TearDown() override {
    if (session) MY_QDMI_device_session_free(session);
...
protected:
  void SetUp() override {
    ...
    ASSERT_EQ(MY_QDMI_device_session_set_parameter(session, QDMI_DEVICE_SESSION_PARAMETER_TOKEN, token.size(), token.c_str()), QDMI_SUCCESS);
...
TEST_F(QDMIInitializedSessionTest, QueryProperties) {
  ...
  EXPECT_EQ(MY_QDMI_device_session_query_device_property(nullptr, QDMI_DEVICE_PROPERTY_NAME, 0, nullptr, &size), QDMI_ERROR_INVALIDARGUMENT);

  // First call: retrieve the required buffer size
  ASSERT_EQ(MY_QDMI_device_session_query_device_property(session, QDMI_DEVICE_PROPERTY_NAME, 0, nullptr, &size), QDMI_SUCCESS)
  ...
  if (size > 1) {
    EXPECT_EQ(MY_QDMI_device_session_query_device_property(session, QDMI_DEVICE_PROPERTY_NAME, small_buffer.size(), small_buffer.data(), nullptr), QDMI_ERROR_INVALIDARGUMENT);
  }

  // Second call: retrieve the actual name
  std::string value(size, '\0');
  ASSERT_EQ(MY_QDMI_device_session_query_device_property(session, QDMI_DEVICE_PROPERTY_NAME, size, value.data(), nullptr), QDMI_SUCCESS);
  EXPECT_GT(value.size(), 0) << "Checkpoint 3 Failed: Name should not be empty.";

  // Unsupported property must return NOTSUPPORTED
  EXPECT_EQ(MY_QDMI_device_session_query_device_property(session, QDMI_DEVICE_PROPERTY_MAX, 0, nullptr, &size), QDMI_ERROR_NOTSUPPORTED);
}
...
TEST_F(QDMIInitializedSessionTest, SubmitAndSimulateJob) {
  ...
  // Null session must return INVALIDARGUMENT
  EXPECT_EQ(MY_QDMI_device_session_create_device_job(nullptr, &job), QDMI_ERROR_INVALIDARGUMENT);
  // Null job-pointer must return INVALIDARGUMENT
  EXPECT_EQ(MY_QDMI_device_session_create_device_job(session, nullptr), QDMI_ERROR_INVALIDARGUMENT);

  ASSERT_EQ(MY_QDMI_device_session_create_device_job(session, &job), QDMI_SUCCESS)
  ...
  EXPECT_EQ(MY_QDMI_device_job_set_parameter(nullptr, QDMI_DEVICE_JOB_PARAMETER_PROGRAM, qasm.size(), qasm.c_str()), QDMI_ERROR_INVALIDARGUMENT);
  // Null value with size==0 must return INVALIDARGUMENT
  EXPECT_EQ(MY_QDMI_device_job_set_parameter(job, QDMI_DEVICE_JOB_PARAMETER_PROGRAM, 0, nullptr), QDMI_ERROR_INVALIDARGUMENT);
  // Unsupported parameter must return NOTSUPPORTED
  EXPECT_EQ(MY_QDMI_device_job_set_parameter(job, QDMI_DEVICE_JOB_PARAMETER_MAX, qasm.size(), qasm.c_str()), QDMI_ERROR_NOTSUPPORTED);

  // Submitting without a program set must return BADSTATE
  EXPECT_EQ(MY_QDMI_device_job_submit(job), QDMI_ERROR_BADSTATE);

  ASSERT_EQ(MY_QDMI_device_job_set_parameter(job, QDMI_DEVICE_JOB_PARAMETER_PROGRAM, qasm.size(), qasm.c_str()), QDMI_SUCCESS);
  ...
  // Null job must return INVALIDARGUMENT
  EXPECT_EQ(MY_QDMI_device_job_check(nullptr, &status), QDMI_ERROR_INVALIDARGUMENT);
  ...
  double probs[2];
  // Null job must return INVALIDARGUMENT
  EXPECT_EQ(MY_QDMI_device_job_get_results(nullptr, QDMI_JOB_RESULT_PROBABILITIES_DENSE, sizeof(probs), probs, nullptr), QDMI_ERROR_INVALIDARGUMENT);
  // Buffer too small must return INVALIDARGUMENT
  EXPECT_EQ(MY_QDMI_device_job_get_results(job, QDMI_JOB_RESULT_PROBABILITIES_DENSE, sizeof(probs) - 1, probs, nullptr), QDMI_ERROR_INVALIDARGUMENT);
  // Unsupported result type must return NOTSUPPORTED
  EXPECT_EQ(MY_QDMI_device_job_get_results(job, QDMI_JOB_RESULT_MAX, sizeof(probs), probs, nullptr), QDMI_ERROR_NOTSUPPORTED);

  ASSERT_EQ(MY_QDMI_device_job_get_results(job, QDMI_JOB_RESULT_PROBABILITIES_DENSE, sizeof(probs), probs, nullptr), QDMI_SUCCESS)
