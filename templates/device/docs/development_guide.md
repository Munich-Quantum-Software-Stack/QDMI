...
- the flag `-DCMAKE_BUILD_TYPE=Release` tells CMake to configure a _Release_ build (as opposed to,
  e.g., a _Debug_ build).
- the flag `-DBUILD_MY_QDMI_TESTS=ON` enables building the tests.
- the flag `-DMY_QDMI_COVERAGE=ON` can be added to enable code coverage support (see below).
- the flag `-DBUILD_MY_QDMI_DOCS=ON` can be added to enable building the documentation (see below).
...
To generate a code coverage report, you can configure the project with `-DMY_QDMI_COVERAGE=ON` and
...
project with CMake and the `-DBUILD_MY_QDMI_DOCS=ON` flag, you can build the documentation by
running:
