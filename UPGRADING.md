...
A `QDMI_DEVICE_SESSION_PARAMETER_TIMEOUT` parameter has been added that influences how long (in
seconds) the `QDMI_device_job_wait` function must wait for a result before returning with a
`QDMI_ERROR_TIMEOUT` error.
...
A `QDMI_SESSION_PARAMETER_TIMEOUT` parameter has been added that influences how long (in seconds)
the `QDMI_job_wait` function must wait for a result before returning with a `QDMI_ERROR_TIMEOUT`
error.
...
A `QDMI_SESSION_PARAMETER_TIMEOUT` parameter has been added that influences how long (in seconds)
the `QDMI_job_wait` will wait before eventually returning with `QDMI_ERROR_TIMEOUT`.
