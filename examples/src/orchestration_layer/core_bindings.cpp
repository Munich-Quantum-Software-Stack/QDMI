...
namespace {
/// @copydoc O_QDMI_context_query_id
auto Context_query_id(Context *context, const size_t size, char *value,
  ...
  context->logger.log(QDMI_LOG_LEVEL_INFO,
                      "[" QDMI_DEVICE_ID "] Querying the ID");
  const auto &id = OrchestrationLayer::get().id();
  if (value != nullptr) {
    if (size < id.size() + 1) {
...
/// @copydoc O_QDMI_context_query_name
auto Context_query_name(Context *context, const size_t size, char *value,
  ...
  context->logger.log(QDMI_LOG_LEVEL_INFO,
                      "[" QDMI_DEVICE_ID "] Querying the name");
  const auto &name = OrchestrationLayer::get().name();
  if (value != nullptr) {
    if (size < name.size() + 1) {
      context->logger.log(QDMI_LOG_LEVEL_ERROR,
                          "[" QDMI_DEVICE_ID "] Buffer too small");
...
/// @copydoc O_QDMI_context_query_version
auto Context_query_version(Context *context, size_t *value) -> int {
  if (context == nullptr) {
    return QDMI_ERROR_INVALIDARGUMENT;
  }
  context->logger.log(QDMI_LOG_LEVEL_INFO,
                      "[" QDMI_DEVICE_ID "] Querying the version");
  if (value != nullptr) {
    *value = OrchestrationLayer::get().version();
  }
  return QDMI_SUCCESS;
}
/// @copydoc O_QDMI_context_query_authentication_options
auto Context_query_authentication_options(Context *context, const size_t size,
  ...
  context->logger.log(QDMI_LOG_LEVEL_INFO,
                      "[" QDMI_DEVICE_ID
                      "] Querying the authentication options");
  const auto &options = OrchestrationLayer::get().authentication_options();
  if (value != nullptr) {
...
/// @copydoc O_QDMI_context_query_modules
auto Context_query_modules(Context *context, const size_t size, Module **values,
  ...
  context->logger.log(QDMI_LOG_LEVEL_INFO,
                      "[" QDMI_DEVICE_ID "] Querying the modules");
  const auto &modules = OrchestrationLayer::get().modules();
  if (values != nullptr) {
...
/// @copydoc O_QDMI_context_query_module_by_id
auto Context_query_module_by_id(Context *context, const char *id,
  ...
  if (*module = OrchestrationLayer::get().module_by_id(id);
      *module == nullptr) {
    context->logger.log(QDMI_LOG_LEVEL_ERROR,
...
/// @copydoc O_QDMI_context_query_module_id
auto Context_query_module_id(Context *context, Module *module,
  ...
  context->logger.log(QDMI_LOG_LEVEL_INFO,
                      "[" QDMI_DEVICE_ID "] Querying the module's ID");
  if (module == nullptr) {
    context->logger.log(QDMI_LOG_LEVEL_ERROR,
                        "[" QDMI_DEVICE_ID "] Invalid arguments");
    return QDMI_ERROR_INVALIDARGUMENT;
...
/// @copydoc O_QDMI_context_get_module_interface
...
/// @copydoc O_QDMI_context_allocate_session
auto Context_allocate_session(Context *context,
  ...
  *session = OrchestrationLayer::get().create_new_session(*context, logger);
  return QDMI_SUCCESS;
}
/// @copydoc O_QDMI_session_set_token
auto Session_set_token(Session *session, const char *value) -> int {
  ...
  if (session->status != Session::STATUS::ALLOCATED) {
    session->logger.log(QDMI_LOG_LEVEL_ERROR,
                        "[" QDMI_DEVICE_ID "] Session is in an invalid state "
...
/// @copydoc O_QDMI_session_set_authentication_file
auto Session_set_authentication_file(Session *session,
  ...
  session->logger.log(QDMI_LOG_LEVEL_ERROR,
                      "[" QDMI_DEVICE_ID
                      "] Authentication file support is not supported");
  return QDMI_ERROR_NOTSUPPORTED;
}
/// @copydoc O_QDMI_session_set_authentication_url
auto Session_set_authentication_url(Session *session,
  ...
  session->logger.log(QDMI_LOG_LEVEL_ERROR,
                      "[" QDMI_DEVICE_ID
                      "] Authentication URL support is not supported");
  return QDMI_ERROR_NOTSUPPORTED;
}
/// @copydoc O_QDMI_session_set_username
auto Session_set_username(Session *session, [[maybe_unused]] const char *value)
  ...
  session->logger.log(QDMI_LOG_LEVEL_ERROR,
                      "[" QDMI_DEVICE_ID "] Username support is not supported");
  return QDMI_ERROR_NOTSUPPORTED;
}
/// @copydoc O_QDMI_session_set_password
auto Session_set_password(Session *session, [[maybe_unused]] const char *value)
  ...
  session->logger.log(QDMI_LOG_LEVEL_ERROR,
                      "[" QDMI_DEVICE_ID "] Password support is not supported");
  return QDMI_ERROR_NOTSUPPORTED;
}
/// @copydoc O_QDMI_session_set_log_callback
...
/// @copydoc O_QDMI_session_initialize
...
/// @copydoc O_QDMI_session_free
auto Session_free(Session *session) -> void {
  if (session != nullptr) {
    session->logger.log(QDMI_LOG_LEVEL_INFO,
                        "[" QDMI_DEVICE_ID "] Freeing the session");
    OrchestrationLayer::get().remove_session(*session);
  }
}
/// @copydoc O_QDMI_context_finalize
auto Context_finalize(Context *context) -> int {
  ...
  OrchestrationLayer::get().remove_context(*context);
  return QDMI_SUCCESS;
}
/// @copydoc O_QDMI_context_set_log_callback
auto Context_set_log_callback(Context *context,
  ...
  context->logger.log(QDMI_LOG_LEVEL_INFO,
                      "[" QDMI_DEVICE_ID
                      "] Setting the context's logging callback");
  if (callback == nullptr && user_data != nullptr) {
    context->logger.log(QDMI_LOG_LEVEL_ERROR,
                        "[" QDMI_DEVICE_ID
                        "] User data provided without a callback function");
...
/// @copydoc O_QDMI_get_interface
auto Get_interface(O_QDMI_Core_Interface const **interface_ptr) -> int {
  if (interface_ptr == nullptr) {
    return QDMI_ERROR_INVALIDARGUMENT;
  }
  *interface_ptr = &OrchestrationLayer::get().core_interface();
  return QDMI_SUCCESS;
}
} // namespace
auto Get_core_interface() -> O_QDMI_Core_Interface {
...
auto Get_library() -> O_QDMI_Library {
  return {.get_interface = &Get_interface,
          .context_set_log_callback = &Context_set_log_callback,
          .context_finalize = &Context_finalize};
}
} // namespace qdmi::orchestration_layer
int O_QDMI_initialize(const size_t version, QDMI_Log_Callback callback,
                      void *user_data,
                      qdmi::orchestration_layer::Context **context,
                      O_QDMI_Library const **interface_ptr) {
  const qdmi::orchestration_layer::Logger logger{callback, user_data};
  ...
  if (version != QDMI_VERSION) {
    if (callback != nullptr) {
      logger.log(
          ...
          QDMI_MAJOR_VERSION(QDMI_VERSION), QDMI_MINOR_VERSION(QDMI_VERSION),
  ...
  auto &orchestration_layer =
      qdmi::orchestration_layer::OrchestrationLayer::get();
  *context = orchestration_layer.create_new_context(logger);
  *interface_ptr = &orchestration_layer.library();
  return QDMI_SUCCESS;
}
#ifndef QDMI_STATIC_DEFINE
const char *QDMI_get_prefix() { return "O"; }
