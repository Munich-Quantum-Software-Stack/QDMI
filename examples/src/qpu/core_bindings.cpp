...
namespace qdmi::qpu {
namespace {
/// @copydoc Q_QDMI_context_query_id
...
/// @copydoc Q_QDMI_context_query_name
...
/// @copydoc Q_QDMI_context_query_version
auto Context_query_version(Context *context, size_t *value) -> int {
  ...
  if (value != nullptr) {
    *value = QPU::get().version();
  }
  return QDMI_SUCCESS;
}
/// @copydoc Q_QDMI_context_query_authentication_options
auto Context_query_authentication_options(Context *context, const size_t size,
  ...
  const auto &options = QPU::get().authentication_options();
  if (value != nullptr) {
...
/// @copydoc Q_QDMI_context_query_modules
auto Context_query_modules(Context *context, const size_t size, Module **values,
  ...
  const auto &modules = QPU::get().modules();
  if (values != nullptr) {
...
/// @copydoc Q_QDMI_context_query_module_by_id
auto Context_query_module_by_id(Context *context, const char *id,
  ...
  if (*module = QPU::get().module_by_id(id); *module == nullptr) {
    context->logger.log(QDMI_LOG_LEVEL_ERROR,
...
/// @copydoc Q_QDMI_context_query_module_id
...
/// @copydoc Q_QDMI_context_get_module_interface
...
/// @copydoc Q_QDMI_context_allocate_session
auto Context_allocate_session(Context *context,
  ...
  *session = QPU::get().create_new_session(context, logger);
  return QDMI_SUCCESS;
}
/// @copydoc Q_QDMI_session_set_token
...
/// @copydoc Q_QDMI_session_set_authentication_file
...
/// @copydoc Q_QDMI_session_set_authentication_url
...
/// @copydoc Q_QDMI_session_set_username
...
/// @copydoc Q_QDMI_session_set_password
...
/// @copydoc Q_QDMI_session_set_log_callback
...
/// @copydoc Q_QDMI_session_initialize
...
/// @copydoc Q_QDMI_session_free
auto Session_free(Session *session) -> void {
  if (session != nullptr) {
    session->logger.log(QDMI_LOG_LEVEL_INFO,
                        "[" QDMI_DEVICE_ID "] Freeing the session");
    QPU::get().remove_session(session);
  }
}
/// @copydoc Q_QDMI_context_finalize
auto Context_finalize(Context *context) -> int {
  ...
  QPU::get().remove_context(context);
  return QDMI_SUCCESS;
}
/// @copydoc Q_QDMI_context_set_log_callback
...
/// @copydoc Q_QDMI_get_interface
auto Get_interface(Q_QDMI_Core_Interface const **interface_ptr) -> int {
  if (interface_ptr == nullptr) {
    return QDMI_ERROR_INVALIDARGUMENT;
  }
  *interface_ptr = &QPU::get().core_interface();
  return QDMI_SUCCESS;
}
} // namespace
auto Get_core_interface() -> Q_QDMI_Core_Interface {
...
auto Get_library() -> Q_QDMI_Library {
  return {.get_interface = &Get_interface,
          .context_set_log_callback = &Context_set_log_callback,
          .context_finalize = &Context_finalize};
}
} // namespace qdmi::qpu
int Q_QDMI_initialize(const size_t version, QDMI_Log_Callback callback,
                      void *user_data, qdmi::qpu::Context **context,
                      Q_QDMI_Library const **interface_ptr) {
  const qdmi::qpu::Logger logger{callback, user_data};
  ...
  if (version != QDMI_VERSION) {
    if (callback != nullptr) {
      logger.log(
          ...
          QDMI_VERSION, QDMI_MAJOR_VERSION(QDMI_VERSION),
          QDMI_MINOR_VERSION(QDMI_VERSION), QDMI_PATCH_VERSION(QDMI_VERSION),
          QDMI_MAJOR_VERSION(version), QDMI_MINOR_VERSION(version),
          QDMI_PATCH_VERSION(version), version);
    }
    return QDMI_ERROR_VERSIONMISMATCH;
  }
  *context = qdmi::qpu::QPU::get().create_new_context(logger);
  *interface_ptr = &qdmi::qpu::QPU::get().library();
  return QDMI_SUCCESS;
}
#ifndef QDMI_STATIC_DEFINE
const char *QDMI_get_prefix() { return "Q"; }
