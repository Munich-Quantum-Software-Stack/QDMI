...
namespace {
/// @copydoc P_QDMI_session_query_devices
auto Session_query_devices(Session *session, size_t size, Device **value,
  ...
  const auto &devices = Provider::get().devices();
  if (value != nullptr) {
...
/// @copydoc P_QDMI_session_query_device_by_id
auto Session_query_device_by_id(Session *session, const char *id,
  ...
  if (*device = Provider::get().device_by_id(id); *device == nullptr) {
    session->logger.log(QDMI_LOG_LEVEL_ERROR,
...
/// @copydoc P_QDMI_session_get_device_interface
auto Session_get_device_interface(Session *session, Device *device,
  ...
  session->logger.log(QDMI_LOG_LEVEL_INFO,
                      "[" QDMI_DEVICE_ID "] Querying a device's interface");
  if (session->status != Session::STATUS::INITIALIZED) {
    session->logger.log(QDMI_LOG_LEVEL_ERROR,
                        "[" QDMI_DEVICE_ID "] Session is not initialized");
    return QDMI_ERROR_BADSTATE;
  ...
  *interface_ptr = device->core_interface;
  return QDMI_SUCCESS;
}
/// @copydoc P_QDMI_session_query_device_context
auto Session_query_device_context(Session *session, Device *device,
                                  QDMI_Context *context) -> int {
  if (session == nullptr) {
    return QDMI_ERROR_INVALIDARGUMENT;
  }
  session->logger.log(QDMI_LOG_LEVEL_INFO,
                      "[" QDMI_DEVICE_ID "] Querying a device's context");
  if (session->status != Session::STATUS::INITIALIZED) {
    session->logger.log(QDMI_LOG_LEVEL_ERROR,
                        "[" QDMI_DEVICE_ID "] Session is not initialized");
    return QDMI_ERROR_BADSTATE;
  ...
  auto library_copy{device->library};
  library_copy.set_logging_callback(session->logger.callback(),
                                    session->logger.user_data());
  *context = library_copy.context();
  session->open_libraries.emplace_back(std::move(library_copy));
  return QDMI_SUCCESS;
}
} // namespace

auto Get_provider_interface() -> P_QDMI_Provider_Interface {
