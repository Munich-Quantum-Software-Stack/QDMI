...
- the flag `-DQDMI_ENABLE_COVERAGE=ON` can be added to enable code coverage support (see below).
...
To generate a code coverage report, you can configure the project with `-DQDMI_ENABLE_COVERAGE=ON` and
...
project with CMake and the `-DBUILD_CXX_QDMI_DOCS=ON` flag, you can build the documentation by
running:
